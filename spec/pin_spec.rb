...
describe Pin do
  ...
  describe "#connect" do
    ...
    it "is only possible to connect other pins" do
      expect { pin.connect(true) }.to raise_error(InvalidPin, "'true' is not a pin")
    end
    it "is not possible to connect a pin twice" do
      expect { pin.connect(pin2); pin.connect(pin2) }.to raise_error(InvalidPin, "Pin is already connected")
    end
  end
end
